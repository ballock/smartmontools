...
bool parse_dev_state_line(const char * line, persistent_dev_state & state);

// Read a state file.
bool read_dev_state(const char * path, persistent_dev_state & state);

void write_dev_state_line(FILE * f, const char * name, uint64_t val);

void write_dev_state_line(FILE * f, const char * name1, int id, const char * name2, uint64_t val);

// Write a state file
bool write_dev_state(const char * path, const persistent_dev_state & state);

// Write to the attrlog file
bool write_dev_attrlog(const char * path, const dev_state & state);

// Write all state files. If write_always is false, don't write
// unless must_write is set.
void write_all_dev_states(const dev_config_vector & configs,
                          dev_state_vector & states,
                          bool write_always = true);

// Write to all attrlog files
void write_all_dev_attrlogs(const dev_config_vector & configs,
                            dev_state_vector & states);
...
void MailWarning(const dev_config & cfg, dev_state & state, int which, const char *fmt, ...)
                 __attribute_format_printf(4, 5);

void reset_warning_mail(const dev_config & cfg, dev_state & state, int which, const char *fmt, ...)
                        __attribute_format_printf(4, 5);
...
int CloseDevice(smart_device * device, const char * name);

// return true if a char is not allowed in a state file name
bool not_allowed_in_filename(char c);

// Read error count from Summary or Extended Comprehensive SMART error log
// Return -1 on error
int read_ata_error_count(ata_device * device, const char * name,
...
int SelfTestErrorCount(ata_device * device, const char * name,
...
inline bool is_offl_coll_in_progress(unsigned char status);

// Check self-test execution status
inline bool is_self_test_in_progress(unsigned char status);

// Log offline data collection status
void log_offline_data_coll_status(const char * name, unsigned char status);

// Log self-test execution status
void log_self_test_exec_status(const char * name, unsigned char status);

// Check pending sector count id (-C, -U directives).
bool check_pending_id(const dev_config & cfg, const dev_state & state,
                      unsigned char id, const char * msg);

// Called by ATA/SCSIDeviceScan() after successful device check
void finish_device_scan(dev_config & cfg, dev_state & state);

// Common function to format result message for ATA setting
void format_set_result_msg(std::string & msg, const char * name, bool ok,
...
int ATADeviceScan(dev_config & cfg, dev_state & state, ata_device * atadev);

// on success, return 0. On failure, return >0.  Never return <0,
// please.
int SCSIDeviceScan(dev_config & cfg, dev_state & state, scsi_device * scsidev);

// If the self-test log has got more self-test errors (or more recent
// self-test errors) recorded, then notify user.
void CheckSelfTestLogs(const dev_config & cfg, dev_state & state, int newi);
...
char next_scheduled_test(const dev_config & cfg, dev_state & state, bool scsi, time_t usetime = 0);

// Print a list of future tests.
void PrintTestSchedule(const dev_config_vector & configs, dev_state_vector & states, const smart_device_list & devices);
...
int DoSCSISelfTest(const dev_config & cfg, dev_state & state, scsi_device * device, char testtype);

// Do an offline immediate or self-test.  Return zero on success,
// nonzero on failure.
int DoATASelfTest(const dev_config & cfg, dev_state & state, ata_device * device, char testtype);

// Check pending sector count attribute values (-C, -U directives).
void check_pending(const dev_config & cfg, dev_state & state,
                   unsigned char id, bool increase_only,
                   const ata_smart_values & smartval,
                   int mailtype, const char * msg);

// Format Temperature value
const char * fmt_temp(unsigned char x, char (& buf)[20]);

// Check Temperature limits
void CheckTemperature(const dev_config & cfg, dev_state & state, unsigned char currtemp, unsigned char triptemp);
...
int ATACheckDevice(const dev_config & cfg, dev_state & state, ata_device * atadev,
                   bool firstpass, bool allow_selftests);

int SCSICheckDevice(const dev_config & cfg, dev_state & state, scsi_device * scsidev, bool allow_selftests);

// 0=not used, 1=not disabled, 2=disable rejected by OS, 3=disabled
int standby_disable_state;

void init_disable_standby_check(dev_config_vector & configs);

void do_disable_standby_check(const dev_config_vector & configs, const dev_state_vector & states);

// Checks the SMART status of all ATA and SCSI devices
void CheckDevicesOnce(const dev_config_vector & configs, dev_state_vector & states,
...
time_t dosleep(time_t wakeuptime, bool & sigwakeup);
...
const char * strtok_dequote(const char * delimiters);
...
int ParseToken(char * token, dev_config & cfg);
...
int ParseConfigLine(dev_config_vector & conf_entries, dev_config & default_conf, int lineno, /*const*/ char * line);
...
int ParseConfigFile(dev_config_vector & conf_entries);
...
void check_abs_path(char option, const std::string & path);
#endif // !_WIN32
...
int MakeConfigEntries(const dev_config & base_cfg,
                      dev_config_vector & conf_entries,
                      smart_device_list & scanned_devs, const char * type);
 
void CanNotRegister(const char *name, const char *type, int line, bool scandirective);

// Returns negative value (see ParseConfigFile()) if config file
// had errors, else number of entries which may be zero or positive. 
int ReadOrMakeConfigEntries(dev_config_vector & conf_entries, smart_device_list & scanned_devs);

// Return true if TYPE contains a RAID drive number
bool is_raid_type(const char * type);

// Return true if DEV is already in DEVICES[0..NUMDEVS) or IGNORED[*]
bool is_duplicate_device(const smart_device * dev,
