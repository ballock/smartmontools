...
char next_scheduled_test(const dev_config & cfg, dev_state & state, bool scsi, time_t usetime)
{
...
void Initialize(time_t *wakeuptime)
{
  ...
  // initialize caughtsig variables
  caughtsigUSR1 = 0;
#ifdef _WIN32
  caughtsigUSR2 = 0;
#endif
...
#endif
{
  ...
  try {
    // Do the real work ...
    status = smartd_main_worker(argc, argv);
  }
  catch (int ex) {
